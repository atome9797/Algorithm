...

{
	...
	//안쪽 원, 바깥쪽 원, 두 원 모두 방어됬는가?

			//이전 블록과 묶어서 방어 가능한가?

	inner =
		(cirTagon[idx][0] + cirTagon[idx ? idx - 1 : N - 1][0] <= sfNum); //idx가 0인 경우 이전 블록이 last

	outer =
		(cirTagon[idx][1] + cirTagon[idx ? idx - 1 : N - 1][1] <= sfNum);

	//위 아래 블록 모두 방어 가능한가?

	both =
		(cirTagon[idx][0] + cirTagon[idx][1] <= sfNum);
...

	{
		...
		if (idx == 0) //원타곤의 구역이 두개 밖에 없었을 경우
			...
			return both ? 1 : 2; //위 아래 모두 방어되어있을 경우 1, 아닐
	...
	if (inner &&
		...
		result
		= min(result, 1 + covered(idx + 1, 1, idx ? last : 1));
	...
	if (outer && prev < 2)
		...
		result
		= min(result, 1 + covered(idx + 1, 2, idx ? last : 2));
	...
	if (inner &&
		...
		result
		= min(result, covered(idx + 1, 3, idx ? last : 3));
	...
	if (both)
		...
		result
		= min(result, 1 + covered(idx + 1, 3, idx ? last : 0));
